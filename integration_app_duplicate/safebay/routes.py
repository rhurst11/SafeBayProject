...
from safebay.forms import RegistrationForm, LoginForm, UpdateAccountForm, Face_RegistrationForm
...
def facereg():
    ...
    if request.method == "POST":
        ...
        name = db.session.execute("SELECT * FROM users WHERE username = :username",
                        username=username)
